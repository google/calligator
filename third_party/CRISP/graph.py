...
class Graph():
  ...
  It is the central data structure on which we compute the critical path. Each
  Graph is built from some Jaeger JSON file represented by its filename. A
  well-formed Jaeger trace should have one and only one rootNode.
  """
  ...
  def __init__(
      ...
      filename,
      rootTrace,
      sanitize_overflowing_children=True,
  ) -> None:
    ...
    if rootTrace == True:  # the root span must be the service+operation
      if len(potentialRoots) != 1:
        ...
        return
      if (
          potentialRoots[0] == None
          or self.checkRootAndWarn(potentialRoots[0], filename, rootTrace)
          == False
      ):
        return
      else:
      ...
      for candidate in potentialRoots:
        ...
        if (
            someRoot == None
            or self.checkRootAndWarn(someRoot, filename, rootTrace) == False
        ):
          continue
      ...
      if self.rootNode == None:
        logging.warning(
            f"rootTrace == {rootTrace} but no matching node found in file"
            f" {filename}!"
        )
        return

    if sanitize_overflowing_children:
    ...
    if debug_on:
      ...
      logging.debug(f"total executionTime {self.rootNode.duration}")  # type: ignore[attribute-error]
  ...
  def computeGraphStats(self, node):
    ...
    descendants = 0
    depth = 0
    for c in node.children:  # type: ignore[attribute-error]
  ...
  def checkRootAndWarn(self, node, filename, rootTrace):
    if self.processName[node.pid] != self.serviceName or node.opName != self.operationName:  # type: ignore[attribute-error]
      logging.warning(
          f"rootTrace == {rootTrace}, expected serviceName={self.serviceName} and found {self.processName[node.pid]}. Expected operationName={self.operationName} and found {node.opName} in file {filename}"  # type: ignore[attribute-error]
      )
      return False
    return True
  ...
  def checkResultsWithoutQuantity(self, cp):
    ...
    return checkResults(self, cp, False)  # type: ignore[name-error]
    ...
    for spanId in self.nodeHT:
      ...
      if parentId not in self.nodeHT:
        debug_on and logging.debug(
            f"Span {spanId}'s parent {parentId} not present in nodeHT: file ="
            f" {self.filename}"
        )
        potentialRoots.append(me)
        continue
  ...
  def sanitizeOverflowingChildren(self, curNode):
    ...
    parentStart = curNode.startTime  # type: ignore[attribute-error]
    parentEnd = curNode.endTime  # type: ignore[attribute-error]

    removeList = []
    for c in curNode.children:  # type: ignore[attribute-error]
    ...
    for r in removeList:
      r.parent = None
      del curNode.children[r]  # type: ignore[attribute-error]

  def dumpShrinkStats(self, curNode, child, shrunk):
    logging.debug(
        f" shrunk node {curNode} duration {curNode.duration} by {shrunk}: child"
        f" {child} => {shrunk / curNode.duration * 100}  reduction"
    )

  def dumpDeletionStats(self, curNode, child, shrunk):
    if curNode.duration > 0:
      logging.debug(
          f" delete: parent node {curNode} duration {curNode.duration} with"
          f" child {child} of {shrunk} => {shrunk / curNode.duration * 100} "
          " reduction"
      )
  ...
  def computeCriticalPath(self, curNode):
    ...
    if len(curNode.children) == 0:  # type: ignore[attribute-error]
    ...
    sortedChildren = sorted(
        curNode.children,  # type: ignore[attribute-error]
  ...
  def happensBefore(
      self, parent, reverseSortedChildren, childBefore, childLater
  ):
  ...
  def getMetrics(self, criticalPath):
    ...
    return Metrics(
        ...
        inclusiveExampleMap,
        callChain,
        self.rootNode.sid,  # type: ignore[attribute-error]
        descendants,
        depth,
    )
